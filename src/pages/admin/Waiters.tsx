...
export default function Waiters() {
    ...
    queryFn: async () => {
      ...
      (paidOrders || []).forEach(order => {
        ...
        if (order.service_fee_enabled && order.subtotal) {
          existing.totalTips += (order.subtotal * (order.service_fee_percentage || 10)) / 100;
        }
  ...
  const openEditDialog = (waiter: Waiter) => {
    ...
    setFormData({ name: waiter.name, phone: waiter.phone || '' });
    setIsDialogOpen(true);
  };

  const toggleActive = (waiter: Waiter) => {
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Switch
                              ...
                              onCheckedChange={() => toggleActive(waiter)}
                            />
