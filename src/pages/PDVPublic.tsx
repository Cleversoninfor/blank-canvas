...
const PDVPublic = () => {
  ...
  const renderProductsGrid = () => {
    if (loadingProducts || loadingCategories) {
      return <div className="col-span-full flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>;
    }
    if (productsError) {
      return <p className="col-span-full text-center text-destructive py-8">Erro ao carregar produtos. Tente novamente.</p>;
    }
    ...
    return filteredProducts.map(product => (
      <Card key={product.id} className="cursor-pointer hover:ring-2 hover:ring-primary/30 transition-all active:scale-[0.97]" onClick={() => addToCart(product)}>
        <CardContent className="p-3 space-y-2">
          {product.image_url && <img src={product.image_url} alt={product.name} className="w-full h-24 object-cover rounded-lg" loading="lazy" />}
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F1F5F9] p-4">
        ...
        <Card className="w-full max-w-sm">
          ...
          <CardContent className="space-y-4">
            {loadingStore ? <div className="flex justify-center py-4"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : (
              <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1">
                <Card className="admin-card border-none shadow-xl sticky top-24">
                  <CardHeader className="bg-primary/5 pb-4"><CardTitle className="flex items-center gap-3 text-lg font-bold"><ShoppingCart className="h-5 w-5" /> Itens do Pedido</CardTitle></CardHeader>
                  <CardContent className="space-y-6 p-6">
                    {cart.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">Clique nos produtos para adicionar</p> : (
                      <>
                        {cart.map(item => (
                        ...
                        <div className="border-t pt-4">
                          <div className="flex justify-between items-center mb-4"><span className="text-muted-foreground font-medium">Total</span><span className="text-2xl font-black text-primary">{formatCurrency(cartTotal)}</span></div>
                          <Button className="w-full h-12 text-lg font-bold" onClick={handleFinalizarPedido} disabled={createOrder.isPending}>Enviar Pedido</Button>
                        </div>
                      </>
                    )}
                  </CardContent>
        ...
        return (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {livres.map(comanda => (
                <Card key={comanda.id} className="admin-card border-none shadow-lg cursor-pointer hover:ring-4 hover:ring-primary/20 transition-all" onClick={() => handleSelectComanda(comanda)}>
                  <CardContent className="p-8 text-center">
                    <div className="bg-green-100 p-3 rounded-full w-fit mx-auto mb-4 text-green-600"><LockOpen className="h-8 w-8" /></div>
                    <p className="font-bold text-xl mb-1">#{comanda.numero_comanda}</p>
                    <Badge variant="default">Livre</Badge>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {ocupadas.map(comanda => <ComandaConsumoCard key={comanda.id} comanda={comanda} onAddMore={(c) => setSelectorComanda(c)} />)}
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {ocupadas.map(comanda => <CloseComandaCard key={comanda.id} comanda={comanda} onClose={() => setCloseSaleComanda(comanda)} onTransfer={() => setTransferSourceComanda(comanda)} />)}
            </div>
          </div>
        );
            ...
            {activeSession && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="flex gap-4 md:col-span-2">
                  <Button variant="outline" className="h-full flex-1 p-6 transition-all bg-white" onClick={() => setSangriaOpen(true)}><ArrowDownCircle className="h-6 w-6 mr-2" /> Realizar Sangria</Button>
                  <Button variant="outline" className="h-full flex-1 p-6 transition-all bg-white" onClick={handleCloseCaixa} disabled={closeCaixa.isPending}><Info className="h-6 w-6 mr-2" /> Fechar Caixa</Button>
                </div>
              </div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <Card className="admin-card border-none shadow-xl cursor-pointer hover:ring-4 hover:ring-primary/20 transition-all group" onClick={() => setView('select-comanda')}>
                <CardContent className="p-10 text-center space-y-4">
                  <div className="mx-auto h-20 w-20 rounded-3xl bg-green-500/10 flex items-center justify-center text-green-600"><ShoppingCart className="h-10 w-10" /></div>
                  <div><CardTitle className="text-2xl font-black">Abrir Venda</CardTitle><p className="text-muted-foreground font-medium">Inicie um novo pedido</p></div>
                  <div className="pt-2"><Badge className="bg-green-500 px-4 py-1">{livres.length} Livres</Badge></div>
                </CardContent>
              </Card>
              <Card className="admin-card border-none shadow-xl cursor-pointer hover:ring-4 hover:ring-blue-500/20 transition-all group" onClick={() => setView('consumo')}>
                <CardContent className="p-10 text-center space-y-4">
                  <div className="mx-auto h-20 w-20 rounded-3xl bg-blue-500/10 flex items-center justify-center text-blue-600"><ClipboardList className="h-10 w-10" /></div>
                  <div><CardTitle className="text-2xl font-black">Consumo</CardTitle><p className="text-muted-foreground font-medium">Gerenciar itens em aberto</p></div>
                  <div className="pt-2"><Badge className="bg-blue-500 px-4 py-1">{ocupadas.length} Ocupadas</Badge></div>
                </CardContent>
              </Card>
              <Card className="admin-card border-none shadow-xl cursor-pointer hover:ring-4 hover:ring-amber-500/20 transition-all group" onClick={() => setView('select-close')}>
                <CardContent className="p-10 text-center space-y-4">
                  <div className="mx-auto h-20 w-20 rounded-3xl bg-amber-500/10 flex items-center justify-center text-amber-600 transition-transform group-hover:scale-110 font-black text-2xl italic">R$</div>
                  <div><CardTitle className="text-2xl font-black">Fechar Venda</CardTitle><p className="text-muted-foreground font-medium">Finalizar pagamentos</p></div>
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] p-4 sm:p-6 md:p-10">
      ...
      {transferSourceComanda && (
        <TransferComandaModal sourceComanda={transferSourceComanda} open={!!transferSourceComanda} onClose={() => setTransferSourceComanda(null)} />
      )}
      {selectorComanda && (
